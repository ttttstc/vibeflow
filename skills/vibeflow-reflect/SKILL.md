...
- `task-progress.md` — 逐会话进度和遇到的问题
- `feature-list.json` — 功能总数、废弃功能、优先级分布
- `RELEASE_NOTES.md` — 实际交付内容
- `docs/plans/*-st-report.md` — 测试结果和缺陷摘要
- `.vibeflow/qa-report.md`（如存在）— QA 发现
- `.vibeflow/review-report.md` — 全局审查发现
- `.vibeflow/plan.md` — Plan 阶段完成标记（含 scope decision）
- `.vibeflow/plan-value-review.md` — 初始价值评估结论
- `.vibeflow/plan-eng-review.md` — 工程审查发现
- `.vibeflow/plan-design-review.md` — 设计审查发现
- `.vibeflow/think-output.md` — 初始问题定义
- `git log` — 提交历史和时间线
...
- **计划 vs 实际**：think-output.md 定义的范围 vs RELEASE_NOTES.md 实际交付
...
#### 4b. 新机会
从 think-output.md 的"10x 版本"和 plan-eng-review.md 的扩展建议中回顾。
...
保存到 `.vibeflow/retro-YYYY-MM-DD.md`：
...
生成 `.vibeflow/increment-request.json`：
...
**依赖：** 发布完成
**产出：** `.vibeflow/retro-YYYY-MM-DD.md`，可选 `.vibeflow/increment-request.json`
**链接到：** done（项目完成）或通过 increment-request.json 触发增量流程
