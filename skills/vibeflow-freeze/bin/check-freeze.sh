#!/usr/bin/env bash
# check-freeze.sh — PreToolUse hook for /vibeflow-freeze skill
...
# Use VIBEFLOW_STATE_DIR env var if set, otherwise default to ~/.vibeflow
...
case "$FILE_PATH" in
  ...
  *)
    ...
    printf '{"permissionDecision":"deny","message":"[vibeflow-freeze] Blocked: %s is outside the freeze boundary (%s). Only edits within the frozen directory are allowed."}\n' "$FILE_PATH" "$FREEZE_DIR"
    ;;
esac
