...
在 Claude Code 插件的默认路径中，本 skill 是 **Build 自动实施闭环的核心内部子步骤**。只有在调试单个阶段、重跑某个功能、或处理失败恢复时，才把它当成显式入口。
...
**调用者：** vibeflow-router 的实施自动闭环（默认）或 vibeflow-build-init 的手动 fallback
**调用顺序：**
