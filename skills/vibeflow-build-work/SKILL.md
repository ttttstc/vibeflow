...
`.vibeflow/workflow.yaml` 是**步骤启用的权威来源**。在执行前读取它，跳过标记为禁用的步骤。典型可裁剪步骤：
...
- 读取 `tasks.md` 中当前功能对应的执行级任务块；将其视为已审批、已门禁的 handoff plan，而不是可随意改写的草稿
- 优先读取 feature 条目中的 `build_contract_ref` 与 `design_section`
...
- feature 条目中的文档引用就是正式索引；如引用不足，再补读原始章节
...
- 默认按任务块顺序执行，不临场重新设计任务结构
- 每个任务块应至少包含：`task_id`、`goal`、`exact_file_paths`、`steps`、`verification_steps`、`rollback_note`、`expected_duration_min`
- 如发现当前功能缺少执行级任务块、任务粒度明显过粗、或字段缺失，**停止实现**并回退到 `tasks` 阶段，而不是在 build-work 阶段自行补写一套新计划
- 如需记录本轮执行情况，只允许追加轻量 `execution_notes` / `status`，不得覆写已批准任务的目标、文件范围、验证步骤或回滚说明

### 5-7. TDD 循环（Red -> Green -> Refactor）
**如 workflow.yaml 启用了 tdd：**
...
- **每次循环一个功能** — 防止上下文耗尽
- **严格步骤顺序** — 不跳过，不重排
- **workflow.yaml 为启用步骤的权威** — 被禁用的步骤可跳过
- **不启用也需验证** — 即使 TDD 被禁用，仍需写测试和验证
- **配置门禁在计划前** — 缺配置时不计划不编码
- **只消费已验证计划** — `tasks.md` 是 handoff plan，不是 build-work 临时重写的草稿
