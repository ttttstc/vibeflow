...
1. **读取 SRS 和 UCD（如需）** — 从 `docs/plans/`
...
当存在 `docs/plans/*-srs.md` 且问题已清晰时，**跳过此步骤**，直接进入步骤 1。
...
- 读取 `docs/plans/*-ucd.md`（如已由 router 在 ucd 阶段生成）
...
完成后将结论保存到 `.vibeflow/plan-eng-review.md`。
...
完成后将结论保存到 `.vibeflow/plan-design-review.md`。
...
**依赖：** `docs/plans/*-srs.md`；可选 `docs/plans/*-ucd.md`（如 router 已生成）；可选 `docs/plans/*-brainstorming.md`（问题探索上下文）
**链接到：** vibeflow-build-init（scope decision 通过后）
**产出：**
- `docs/plans/YYYY-MM-DD-<topic>-design.md`（设计文档，含 UI/UX 章节）
- `.vibeflow/plan-eng-review.md`（工程审查结论，AI 独立审查）
- `.vibeflow/plan-design-review.md`（设计审查结论，AI 独立审查）

**UCD 行为说明：**
- SRS 有 UI 需求 + `*-ucd.md` 已存在：读取 UCD 作为输入，跳过 UCD 生成
- SRS 有 UI 需求 + 无 UCD：执行 UCD 子步骤（步骤 1.2）生成风格 Token 和提示词
