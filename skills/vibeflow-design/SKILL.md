...
description: "以审批通过的 context 为输入产出架构/设计文档，回答 HOW。内置问题探索（absorbed from brainstorming）— 当问题不清晰时在设计前先厘清。"
---
...
**此 skill 已吸收 brainstorming 核心行为：当无 context 或问题不清晰时，先进行问题探索再进入技术设计。**
...
0. **[问题探索]** — 如果无 context 或问题不清晰，先厘清问题和方向
1. **读取上下文和 UCD（如需）** — 从 `docs/changes/<change-id>/`
...
9. **过渡到 tasks** — 进入 `vibeflow-tasks`

**终止状态是进入 vibeflow-tasks。**
...
## 步骤 1：读取 Context 和 UCD（如需）并提取设计输入
...
**依赖：** `docs/changes/<change-id>/brief.md`；可选 `docs/changes/<change-id>/ucd.md`；可选 `docs/plans/*-brainstorming.md`（历史问题探索上下文）
