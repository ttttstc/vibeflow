...
8. **过渡到交付计划** — 进入 `vibeflow-build-init` 生成全量交付计划，随后等待人工确认

**终止状态是进入 `vibeflow-build-init`，并在任务拆分后停在 `tasks` 人工确认闸门。**
...
优先检查是否存在 `docs/changes/<change-id>/context.md`：
- **如果存在**：读取其内容作为当前 change 的问题探索结论，包含问题定义、方向选择、开放问题
- **如果不存在**：继续以下探索步骤
...
2. 进入 `vibeflow-build-init` 搭建项目、生成 `feature-list.json`、实施任务包和 `tasks.md`
3. `tasks.md` 必须展示**全量交付计划**（任务拆分、依赖、验证方式、执行顺序）
4. 在用户**人工审核并确认同意**之前，不得进入 `build-config` 或 `build-work`
...
**依赖：** `docs/changes/<change-id>/requirements.md`；可选 `docs/changes/<change-id>/ucd.md`；可选 `docs/changes/<change-id>/context.md`（当前 change 的问题探索上下文）
**链接到：** vibeflow-build-init（scope decision 通过后，生成交付计划并交人工确认）
**产出：**
