...
| Build | 自动接管 | 进入 `build-init` 后默认交给实施闭环，不再逐段等待用户 |
...
返回：`increment`、`think`、`template-selection`、`plan`、`requirements`、`design`、`build-init`、`build-config`、`build-work`、`review`、`test-system`、`test-qa`、`ship`、`reflect`、`done`
...
| `build-init` | 实施自动闭环（Claude Code 默认） / `scripts/run-vibeflow-autopilot.py`（CLI 对应物） | 项目脚手架 |
| `build-config` | 实施自动闭环（默认继续） | `.vibeflow/work-config.json` |
| `build-work` | 实施自动闭环（默认继续） | 已实现功能 |
| `review` | 实施自动闭环（默认继续） | `docs/changes/<change-id>/verification/review.md` |
| `test-system` | 实施自动闭环（默认继续） | `docs/changes/<change-id>/verification/system-test.md` |
| `test-qa` | 实施自动闭环（默认继续） | `docs/changes/<change-id>/verification/qa.md` |
| `ship` | 实施自动闭环（默认继续） | `RELEASE_NOTES.md` |
| `reflect` | 实施自动闭环（默认继续） | `.vibeflow/logs/retro-*.md` |
| `done` | — | 完成摘要 |
...
Claude Code 插件里的默认行为不是停在当前子阶段等用户继续，而是进入一次实施自动闭环：
...
对 Claude Code 来说，**Build 是实施入口，实施闭环是默认执行语义**。  
...
如果用户明确要求命令行无人值守、CI 执行或 dashboard 配套运行，则使用这条实施闭环的脚本入口：
...
### 阶段：`build-init`
**条件**：设计完成，初始化构建。
**操作**：启动实施自动闭环。先执行 `skills/vibeflow-build-init/SKILL.md` 完成初始化，再立即重检 phase 并继续后续阶段。仅在用户明确要求调试单阶段时，才把 `vibeflow-build-init` 当成独立停顿点。

### 阶段：`build-config`
**条件**：build-init 完成，需功能配置。
**操作**：作为实施自动闭环的内部子步骤运行 `python scripts/new-vibeflow-work-config.py`，完成后立即继续下一阶段，不等待用户额外确认。

### 阶段：`build-work`
**条件**：build-config 完成，開始实现功能。
**操作**：作为实施自动闭环的核心执行步骤，使用 `skills/vibeflow-build-work/SKILL.md` 遍历 feature-list.json 按优先级实现功能。若审查/测试失败返回此阶段，自动闭环继续从这里恢复。

### 阶段：`review`
**条件**：功能实现完成，需代码审查。
**操作**：作为实施自动闭环的一部分运行 `skills/vibeflow-review/SKILL.md`，输出 `docs/changes/<change-id>/verification/review.md`。审查失败则自动回到 `build-work` 继续处理。

### 阶段：`test-system`
**条件**：审查通过，需系统测试。
**操作**：作为实施自动闭环的一部分运行 `skills/vibeflow-test-system/SKILL.md`，输出 `docs/changes/<change-id>/verification/system-test.md`。

### 阶段：`test-qa`
**条件**：系统测试通过，需 QA 验证。
**操作**：作为实施自动闭环的一部分运行 `skills/vibeflow-test-qa/SKILL.md`，输出 `docs/changes/<change-id>/verification/qa.md`。

### 阶段：`ship`
**条件**：所有测试通过，部署就绪。
**操作**：作为实施自动闭环的一部分运行 `skills/vibeflow-ship/SKILL.md`，生成 `RELEASE_NOTES.md`，更新 feature-list.json。

### 阶段：`reflect`
**条件**：项目交付完成，需回顾。
**操作**：作为实施自动闭环的一部分运行 `skills/vibeflow-reflect/SKILL.md`，输出 `.vibeflow/logs/retro-*.md`。
...
### 规则3.5：进入 Build 后默认进入实施闭环

当 phase 首次进入 `build-init` 时，默认开始实施自动闭环，并持续推进 `build-init -> build-config -> build-work -> review -> test -> ship -> reflect`。除非用户明确要求单阶段调试，否则不要在这些子阶段之间停下来等待下一条指令。
