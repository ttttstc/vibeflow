...
一旦 Design 已确认，VibeFlow 现在可以直接接管后半程：

- `python scripts/run-vibeflow-autopilot.py --project-root <repo>`
  从当前 phase 自动推进，直到 `done`、遇到阻塞，或停在人工确认点。
...
| `runtime.json` | 运行态覆盖层：autopilot 当前动作、友好提示、最近事件和 heartbeat；dashboard 直接读取它做 live 刷新 |
...
| `feature-list.json` | 功能清单；Build 阶段的单一事实来源，支持依赖、状态、校验步骤和 `autopilot_commands` |
...
│   ├── run-vibeflow-autopilot.py    # 自动接管 Build → Review → Test → Ship → Reflect
...
│   ├── vibeflow_automation.py       # autopilot / build orchestration 核心
