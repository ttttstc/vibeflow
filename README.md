...
VibeFlow 是一个面向 Claude Code 的结构化交付框架。大模型擅长即兴，软件工程需要定力。真正稀缺的不是“生成一段代码”，而是把一个想法稳稳穿过需求、设计、实现、审查、测试、发布与复盘，直到它变成一个可以交付、可以追溯、可以继续演化的系统。VibeFlow 做的，就是给这条路铺上轨道，让 AI 少一点碰运气，多一点章法、记忆与收尾。
...
1. 优先使用官方安装脚本：
   /sh curl -fsSL https://raw.githubusercontent.com/ttttstc/vibeflow/main/claude-code/install.sh | bash
2. 如果脚本不可用，再手动安装到 Claude Code marketplace 目录
3. 安装完成后执行：
   /plugin install vibeflow@vibeflow
4. 最后告诉我：
   - 是否安装成功
