...
│  执行阶段（自动接管）                                          │
│  Build → Review → Test                                      │
│  进入 build-init 后默认由实施闭环继续到结束或阻塞               │
└─────────────────────────────────────────────────────────────┘
                              │
                         ┌────┴────┐
                         ▼         ▼
...
**目标**：把后半程交给实施闭环，而不是让用户手动逐段推进。

在 Claude Code 插件里，进入 `build-init` 后默认开始实施自动闭环；`run-vibeflow-autopilot.py` 只是这条链路的命令行入口。
...
| `runtime.json` | 运行态覆盖层：实施闭环当前动作、友好提示、最近事件和 heartbeat；dashboard 直接读取它做 live 刷新 |
...
│   ├── run-vibeflow-autopilot.py    # 实施闭环的命令行入口
...
│   ├── vibeflow_automation.py       # 实施闭环 / build orchestration 核心
